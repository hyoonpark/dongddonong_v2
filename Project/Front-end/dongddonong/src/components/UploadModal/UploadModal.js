      ...
      <div className="h-[260px] grid grid-rows-9">
        <div className="mt-4 text-center row-start-1">
          <button
            ...
            className=" sm:w-72 border-2 w-48 hover:text-white hover:bg-primary text-primary rounded-lg border-primary "
          >
            업로드할 영상 선택하기
          </button>
          <input
        ...
        <div className="mt-4 row-start-2 row-end-6 grid grid-cols-6 overflow-auto">
          {files &&
            [...files].map((file, index) => (
              <div
                className="h-30 col-start-1 sm:col-start-2 col-end-6 mt-2 gap-4"
                key={file.name}
              >
                <ul>
                  <div className="flex justify-between">
                    <div>{file.name}</div>
                    <div className="">{formatTime(videoDurations[index])}</div>
                  </div>
                      ...
                      {videoDurations[index] < 900 && (
                        <MultiButton
                          ...
                          onChange={(selectedValue) =>
                            handleButtonChange(index, selectedValue)
                          }
                        />
                      )}
                      {videoDurations[index] > 900 && (
        ...
        {files &&
          files.length > 0 && ( // files.length > 0을 추가하니까 사라지네 왜지?
            <div className=" text-right row-start-7">
              {videoDurations.some((duration) => duration > 900) ? (
                ...
                <button
                  ...
                  className={`${
                    files.length === gameTypes.length
                      ? null
                      : "pointer-events-none"
                  } mr-6 mt-4 submit bg-primary text-white  w-32 rounded-lg`}
                >
                  {files.length === gameTypes.length
                    ? "업로드"
                    : "모드 선택필요"}
