  ...
  return (
    <Wrapper>
      <div className="flex flex-col mt-4">
        ...
        <div className="mt-4 h-10 bg-secondary flex text-center justify-evenly items-center rounded">
          {modes.map((option, index) => (
            <button
              key={index}
              className={
                option === mode
                  ? " sm:w-40 bg-primary ml-2 w-20 drop-shadow-xl text-white rounded-md"
                  : "sm:w-40 ml-2 w-20 drop-shadow-xl text-primary bg-white rounded-md"
              }
              onClick={() => {
                setMode(option);
        ...
        {/* 해당 결과 통계 데이터 */}
        <div className="flex flex-col h-60 border-solid border border-black mt-4 bg-white rounded">
          ...
          <div className="flex h-2/3">
            <div className="flex items-center w-2/5 justify-center pb-2">
              <Chart
                scores={{ total2PA, total2PM, total3PA, total3PM }}
                labels={["2점슛", "3점슛"]}
              ></Chart>
            </div>
            ...
            <div className="relative flex justify-between items-center w-3/5">
              <div className="flex flex-col w-1/2 h-full  justify-center">
                ...
                <div className="text-lg text-center h-1/5">
                  {total2PM}/{total2PA}회
                </div>
              </div>
